...
func process(a [][]byte, initialRay Ray) int {
	todo := []Ray{initialRay}
	...
	// fmt.Printf("%v %d items in active map\n", initialRay, len(active))
	return len(active)
}
...
func main() {
	...
	initialRays := []Ray{}
	for i := range a {
		initialRays = append(initialRays, Ray{Position{i, 0}, East})
		initialRays = append(initialRays, Ray{Position{len(a[0]) - 1, 0}, West})
	}

	for i := range a[0] {
		initialRays = append(initialRays, Ray{Position{0, i}, South})
		initialRays = append(initialRays, Ray{Position{len(a), i}, North})
	}

	energized := ick.MapSlice(func(ray Ray) int { return process(a, ray) }, initialRays)
	fmt.Printf("%d\n", ick.Max(energized))
}
